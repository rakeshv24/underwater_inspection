...
bool MapCarving::initialize() {
  ...
  viewpoint_map_pub_ = nh.advertise<geometry_msgs::PoseArray>(viewpoint_map_topic_, 2);
...
void MapCarving::viewpointCallback(const inspection_planner_msgs::Viewpoint::ConstPtr &viewpoint_msg) {
  ...
  obtainViewpointInfo(viewpoint_msg->x, viewpoint_msg->y, viewpoint_msg->z, viewpoint_msg->yaw);
}
...
void MapCarving::publishViewpointInfo() {
...
void MapCarving::obtainViewpointInfo(double vp_x, double vp_y, double vp_z, double vp_yaw){
  ...
  if(tree_initialized_){
    ...
    for(double pitch_direction = -nod_fov/2; pitch_direction <= nod_fov/2; pitch_direction += nod_angle_inc){
      for(double yaw_direction = wrapAngle(vp_yaw-h_fov/2); yaw_direction <= wrapAngle(vp_yaw+h_fov/2); yaw_direction += angle_inc){
        ...
        bool ray_hit_success = collision_tree_->castRay(ray_origin_pose, ray_direction, ray_end_pt, true, 30.0);
    ...
    std::cout<<"[MapCarving][obtainViewpointInfo] Occupied cells: "<<occ_cells_<<std::endl;
